...
test.describe('Test tags-inputs-box', () => {
  ...
  test.skip('TC1: Verify the existence of the tag called node and javascript.', async ({ page }) => {
    await expect(page.locator('text="Tags"')).toBeVisible();
    await expect(page.locator('text=node')).toBeVisible();
    await expect(page.locator('text=javascript')).toBeVisible();
  });

  test.skip('TC2: Remove the tags called node and javascript.', async ({ page }) => {
  ...
  test('TC4: Adds maximum tags (10) with node and javascript pre-existing', async ({page}) => {
 
    const input = page.locator('input[type="text"]');
    let newTags = [ "playwright", "cypress", "python", "selenium",'git','sql','postman','java']
  ...
  test.skip('TC5: Adds maximum tags (10) with node and javascript, then deletes all.', async ({page}) => {
    let newTags = [ "playwright", "cypress", "python", "selenium",'git','sql','postman','java']
    for(let i = 0; i <= newTags.length - 1; i++){
    await input.fill(newTags[i])
    await input.press('Enter')
